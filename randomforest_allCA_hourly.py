# -*- coding: utf-8 -*-
"""
Created on Wed Jan 17 15:32:48 2024

RF for all of the California data combined
(seeing if this makes the model more transferable)
HOURLY
...
for n in range(len(pollutants)):
    #Create a blank dataframe to hold all our input data
    data = pd.DataFrame()
    ...
    y = pd.DataFrame(data.pop('Y_hatfield'))
    ...
    subfolder_name = 'Outputs_{}_rf_allCA_hourly'.format(pollutants[n])
    ...
    #X_train
    savePath = os.path.join(subfolder_path,'allCA_X_train_{}_hourly.csv'.format(pollutants[n]))
    X_train.to_csv(savePath)
        
    #X_test
    savePath = os.path.join(subfolder_path,'allCA_X_test_{}_hourly.csv'.format(pollutants[n]))
    X_test.to_csv(savePath)
        
    #Importance labels
    savePath = os.path.join(subfolder_path,'allCA_Factor_importance_{}_hourly.csv'.format(pollutants[n]))
    ...
    #Stats_train
    savePath = os.path.join(subfolder_path,'allCA_stats_train_{}_hourly.csv'.format(pollutants[n]))
    #Convert the dictionary to a DataFrame
    stats_train = pd.DataFrame.from_dict(stats_train, orient='index', columns=['Value'])
    stats_train.to_csv(savePath)
        
    #Stats_test
    savePath = os.path.join(subfolder_path,'allCA_stats_test_{}_hourly.csv'.format(pollutants[n]))
    #Convert the dictionary to a DataFrame
    ...
    savePath = os.path.join(subfolder_path,'allCA_rfmodel_{}_hourly.joblib'.format(pollutants[n]))
