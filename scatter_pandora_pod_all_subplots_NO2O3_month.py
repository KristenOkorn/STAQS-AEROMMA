# -*- coding: utf-8 -*-
"""
Created on Wed Dec  6 18:39:09 2023
...
for n in range(len(locations)): 
    ...
    sc = ax4[n].scatter(merge['INSTEP O3'], merge['Pandora {} NO2'.format(measTyp)],c=merge['Month'], s=25)
...
fig4.text(0.03, 0.5, 'Pandora {} NO2 ({})'.format(measTyp,unit), va='center', rotation='vertical',fontsize=16)
#Common x-axis label for all subplots
fig4.text(0.5, 0.1, 'INSTEP {} O3 (ppb)'.format(measTyp), ha='center',fontsize=16)
...
#Create the full path with the figure name
if IQR == 'yes':
    savePath = os.path.join(Spath,'Pandora_INSTEP_scatter_NO2O3_{}_IQR_month'.format(measTyp))
else:
    savePath = os.path.join(Spath,'Pandora_INSTEP_scatter_NO2O3_{}_month'.format(measTyp))
