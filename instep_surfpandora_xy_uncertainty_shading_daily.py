# -*- coding: utf-8 -*-
"""
Created on Tue Oct  7 12:01:23 2025

Scatterplot of INSTEP and Pandora surface HCHO data
Shading for uncertainty of each
Daily averages
...
for n in range(len(locations)): 
    ...
    #Time to plot!     
    #-------------------------------------
    axs[n].scatter(merge['INSTEP HCHO'], merge['HCHO'], color=color[n], s=12)
    
    # Add shaded area for uncertainty
    axs[n].fill_between(merge['INSTEP HCHO'] - merge['uncert'], merge['INSTEP HCHO'] + merge['uncert'], merge['HCHO'] - merge['surface_uncertainty'] , merge['HCHO'] + merge['surface_uncertainty'], color=color[n], alpha=0.2, label='Uncertainty')
...
savePath = os.path.join(Spath,'INSTEP vs Pandora Surface with uncertainty daily')
