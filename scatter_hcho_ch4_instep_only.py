# -*- coding: utf-8 -*-
"""
Created on Thu Oct  2 18:58:12 2025

XY Scatterplot of HCHO and CH4
...
for n in range(len(locations)): 
    ...
    x_max = pod_data['INSTEP CH4'].max()
    y_max = pod_data['INSTEP HCHO'].max()
    
    #and use 0 for all mins - 80s are so points don't get cut off at the edges
    x_min = pod_data['INSTEP CH4'].min()
    y_min = pod_data['INSTEP HCHO'].min()
    ...
    axs[n].scatter(pod_data['INSTEP CH4'], pod_data['INSTEP HCHO'], label='INSTEP', color=color[n])
    
    #add a line of best fit to each
    m, b = np.polyfit(pod_data['INSTEP CH4'], pod_data['INSTEP HCHO'], 1)  # slope, intercept
    axs[n].plot(pod_data['INSTEP CH4'], m*pod_data['INSTEP CH4'] + b, color='black')
...
fig.text(0.03, 0.5, 'HCHO (ppb)', va='center', rotation='vertical',fontsize=16)
...
fig.text(0.5, 0.04, 'CH4 (ppm)', ha='center', va='center', fontsize=16)
...
savePath = os.path.join(Spath,'INSTEP_HCHO_CH4')
