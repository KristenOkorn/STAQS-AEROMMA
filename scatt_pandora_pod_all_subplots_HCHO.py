...
for n in range(len(locations)): 
    ...
    pandorafilename = "{}_{}_HCHO.csv".format(locations[n],measTyp)
    ...
    ax4[n].scatter(merge['INSTEP HCHO'], merge['Pandora {} HCHO'.format(measTyp)],c=colors[n], s=25)
...
fig4.text(0.03, 0.5, 'Pandora {} HCHO ({})'.format(measTyp,unit), va='center', rotation='vertical')
#Common x-axis label for all subplots
fig4.text(0.5, 0.09, 'INSTEP {} HCHO (ppb)'.format(measTyp), ha='center')
...
#Create the full path with the figure name
if IQR == 'yes':
    savePath = os.path.join(Spath,'Pandora_INSTEP_scatter_HCHO_{}_IQR'.format(measTyp))
else:
    savePath = os.path.join(Spath,'Pandora_INSTEP_scatter_HCHO_{}'.format(measTyp))
