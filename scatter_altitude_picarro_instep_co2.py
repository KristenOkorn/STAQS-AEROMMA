...
Created on Thu Sep 26 15:37:13 2024

Altitude plot of Picarro CO2 data + INSTEP
...
for n in range(len(locations)): 
    ...
    pod.columns.values[0] = 'INSTEP CO2'
    ...
    for k, (day, df) in enumerate(split_dataframes.items()):
        ...
        axs[k].scatter(df['CO2_ppm'], df['altitude'], label='Picarro', color='black')
        #then plot the instep data
        axs[k].scatter(df['INSTEP CO2'], df['INSTEP altitude'], label='INSTEP', color='red')
    ...
    axs[-1].set_xlabel('CO2 (ppm)', ha='center',fontsize=16)
    ...
    savePath = os.path.join(Spath,'altitude_CO2_{}'.format(locations[n]))
