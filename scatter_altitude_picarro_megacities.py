# -*- coding: utf-8 -*-
"""
Created on Tue May  6 12:40:13 2025
...
This is only for Caltech Megacities data - no pod data these dates
...
#only have megacities at Caltech
locations = ['Caltech'] 
...
for n in range(len(locations)): 
    ...
    podPath = 'C:\\Users\\okorn\\Documents\\2023 AEROMMA\\Megacities'
    #get the filename for the pod
    ch4filename = "CIT-2023-ch4-all-1-hour.csv"
    #read in the first worksheet from the workbook myexcel.xlsx
    podfilepath = os.path.join(podPath, ch4filename)
    podch4 = pd.read_csv(podfilepath,index_col=0)  
    #remove any negatives
    podch4 = podch4[podch4.iloc[:, 11] >= 0]
    #Rename the index to match that of the pandora
    podch4 = podch4.rename_axis('datetime')
    #Convert the index to a DatetimeIndex and set the nanosecond values to zero
    podch4.index = pd.to_datetime(podch4.index)
    #drop the time zone
    podch4.index = podch4.index.tz_localize(None)
    #Change the pollutant column name
    podch4.columns.values[11] = 'Megacities CH4'
    #convert ppb to ppm
    podch4['Megacities CH4'] = podch4['Megacities CH4'] /1000
    #add a column for altitude - will all be 0
    podch4['Megacities altitude'] = 48
    
    #-------------------------------------
    #now load in the matching pod data - CO2
    #get the filename for the pod
    co2filename = "CIT-2023-co2-all-1-hour.csv"
    #read in the first worksheet from the workbook myexcel.xlsx
    podfilepath = os.path.join(podPath, co2filename)
    podco2 = pd.read_csv(podfilepath,index_col=0)  
    #remove any negatives
    podco2 = podco2[podco2.iloc[:, 11] >= 0]
    #Rename the index to match that of the pandora
    podco2 = podco2.rename_axis('datetime')
    #Convert the index to a DatetimeIndex and set the nanosecond values to zero
    podco2.index = pd.to_datetime(podco2.index)
    #drop the time zone
    podco2.index = podco2.index.tz_localize(None)#Change the pollutant column name
    #Change the pollutant column name
    podco2.columns.values[11] = 'Megacities CO2'
    #add a column for altitude - will all be 0
    podco2['Megacities altitude'] = 48
    ...
    x_max_ch4 = math.ceil(max(picarroch4['CH4_ppm'].max(), podch4['Megacities CH4'].max()))
    x_min_ch4 = math.ceil(min(picarroch4['CH4_ppm'].min(), podch4['Megacities CH4'].min())) -0.15
    y_max_ch4 = picarro['altitude'].max() +80

    #and repeat for co2
    x_max_co2 = math.ceil(max(picarroco2['CO2_ppm'].max(), podco2['Megacities CO2'].max()))
    x_min_co2 = math.ceil(min(picarroco2['CO2_ppm'].min(), podco2['Megacities CO2'].min())) -10
    ...
    for k, (day, df) in enumerate(ch4_split_dataframes.items()):   
        axs[k,1].scatter(df['Megacities CH4'], df['Megacities altitude'], label='Megacities', color='red')
    ...
    for k, (day, df) in enumerate(co2_split_dataframes.items()):
        axs[k,0].scatter(df['Megacities CO2'], df['Megacities altitude'], label='Megacities', color='red')
        #Add a title with the date to each subplot
