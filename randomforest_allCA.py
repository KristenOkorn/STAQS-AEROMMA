# -*- coding: utf-8 -*-
"""
Created on Wed Jan 17 12:22:35 2024
...
for n in range(len(pollutants)):
    ...
    subfolder_name = 'Outputs_{}_rf_allCA'.format(pollutants[n])
    ...
    #X_train
    savePath = os.path.join(subfolder_path,'allCA_X_train_{}.csv'.format(pollutants[n]))
    X_train.to_csv(savePath)
        
    #X_test
    savePath = os.path.join(subfolder_path,'allCA_X_test_{}.csv'.format(pollutants[n]))
    X_test.to_csv(savePath)
        
    #Importance labels
    savePath = os.path.join(subfolder_path,'allCA_Factor_importance_{}.csv'.format(pollutants[n]))
    ...
    #Stats_train
    savePath = os.path.join(subfolder_path,'allCA_stats_train_{}.csv'.format(pollutants[n]))
    #Convert the dictionary to a DataFrame
    stats_train = pd.DataFrame.from_dict(stats_train, orient='index', columns=['Value'])
    stats_train.to_csv(savePath)
        
    #Stats_test
    savePath = os.path.join(subfolder_path,'allCA_stats_test_{}.csv'.format(pollutants[n]))
    #Convert the dictionary to a DataFrame
    ...
    savePath = os.path.join(subfolder_path,'allCA_rfmodel_{}.joblib'.format(pollutants[n]))
