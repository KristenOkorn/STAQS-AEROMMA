# -*- coding: utf-8 -*-
"""
Created on Fri Dec  1 14:24:19 2023

Merge ISAF HCHO data with MMS - lat, lon, alt
...
locations = ['AFRC','TMF','Caltech','Whittier','Redlands','St Anthonys','Manhattan Beach','Guenser Park','Elm Avenue','Judson', 'St. Luke','Hudson','Inner Port','First Methodist','Harbor Park']
...
for n in range(len(locations)):
    ...
    ISAFfilePath = 'C:\\Users\\okorn\\Documents\\2023 Aeromma\\ISAF_HCHO.csv'
       
    #load in the file
    ISAF = pd.read_csv(ISAFfilePath,index_col=0)
    
    #make sure the index is a datetime
    ISAF.index = pd.to_datetime(ISAF.index, format='%Y-%m-%d %H:%M:%S')
        
    #------------------------------------------------------------
    #merge the MMS and ISAF data
    merge = pd.merge(MMS,ISAF,left_index=True, right_index=True)
    ...
    savePath = os.path.join(Spath,'ISAF_HCHO_{}.csv'.format(locations[n]))
