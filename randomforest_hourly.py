# -*- coding: utf-8 -*-
"""
Created on Wed Jan 17 13:15:10 2024

Random forest - with HOURLY time averaging
...
for n in range(len(pollutants)):
    for k in range(len(locations)):
        ...
        subfolder_name = 'Outputs_{}_rf_hourly'.format(pollutants[n])
        ...
        #X_train
        savePath = os.path.join(subfolder_path,'{}_X_train_{}_hourly.csv'.format(locations[k],pollutants[n]))
        X_train.to_csv(savePath)
        
        #X_test
        savePath = os.path.join(subfolder_path,'{}_X_test_{}_hourly.csv'.format(locations[k],pollutants[n]))
        X_test.to_csv(savePath)
        
        #Importance labels
        savePath = os.path.join(subfolder_path,'{}_Factor_importance_{}_hourly.csv'.format(locations[k],pollutants[n]))
        ...
        #Stats_train
        savePath = os.path.join(subfolder_path,'{}_stats_train_{}_hourly.csv'.format(locations[k],pollutants[n]))
        # Convert the dictionary to a DataFrame
        stats_train = pd.DataFrame.from_dict(stats_train, orient='index', columns=['Value'])
        stats_train.to_csv(savePath)
        
        #Stats_test
        savePath = os.path.join(subfolder_path,'{}_stats_test_{}_hourly.csv'.format(locations[k],pollutants[n]))
        # Convert the dictionary to a DataFrame
        ...
        savePath = os.path.join(subfolder_path,'{}_rfmodel_{}_hourly.joblib'.format(locations[k],pollutants[n]))
