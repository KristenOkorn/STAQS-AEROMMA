...
for n in range(len(pods)):
    ...
    pod = pod[pod.index.isin(pd.to_datetime(dates))]
    ...
    pod_hcho = (1/podmerge['INSTEP HCHO'])*(2000)*(1/podmerge['temperature'])*(1/0.08206)*(10**-10)*(6.022*(10**-23))
...
for k in range(len(slocations)):
    ...
    #Convert the index to a DatetimeIndex and set the nanosecond values to zero
    scaqmd.index = pd.to_datetime(scaqmd.index,format="%d-%b-%Y %H:%M:%S",errors='coerce')
    #Change the pollutant column name
    scaqmd.columns.values[0] = 'SCAQMD HCHO'
    #now limit the data to just our desired dates
    scaqmd = scaqmd[scaqmd.index.isin(pd.to_datetime(dates))]
    ...
    stemp = pd.read_csv(scaqmdfilepath,index_col=0) 
    #make sure the temperatures are interpreted as numbers
    stemp.iloc[:, 0] = stemp.iloc[:, 0].replace('--', np.nan).astype(float)
    #remove any negatives
    stemp = stemp[stemp.iloc[:, 0] >= 0]
    #Change the temperature column name
    stemp.columns.values[0] = 'temperature'
    #convert from F to K
    stemp['temperature'] = ((5/9)*stemp['temperature']) + 459.67
    #Rename the index to match that of the pandora
    stemp = stemp.rename_axis('datetime')
    #Convert the index to a DatetimeIndex and set the nanosecond values to zero
    stemp.index = pd.to_datetime(stemp.index,format="%d-%b-%Y %H:%M:%S",errors='coerce')
    #now limit the data to just our desired dates
    stemp = stemp[stemp.index.isin(pd.to_datetime(dates))]
    
    #average and merge our datasets
    scaqmd = scaqmd['SCAQMD HCHO'].resample('D').mean()
    scaqmd_merge = pd.merge(scaqmd,stemp,left_index=True, right_index=True)
                         
    #convert the ppb value to molec/cm2
    scaqmd_hcho = (1/scaqmd_merge['SCAQMD HCHO'])*(2000)*(1/scaqmd_merge['temperature'])*(1/0.08206)*(10**-10)*(6.022*(10**-23))
    
    #now get the overall average of converted values
    scaqmd_avg = scaqmd_hcho.mean()
