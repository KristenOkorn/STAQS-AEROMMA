# -*- coding: utf-8 -*-
"""
Created on Fri Oct 27 14:40:16 2023

@author: okorn

Load in nc GCAS files - NO2 data

Keep for individual days - will be too confusing w/ 30 files each
"""
...
for n in range(len(locations)):
    ...
    for i in range(len(fileList)):
        ...
        if match_indices.size != 0:
            ...
            for k in range(len(rows)):
                df.loc[my_datetime[k], 'NO2'] = no2[rows[k],columns[k]]
    ...
    savePath = os.path.join(Spath,'GCAS_NO2_{}.csv'.format(locations[n]))
