# -*- coding: utf-8 -*-
"""
Created on Thu Oct  2 18:40:39 2025

XY Scatterplot of ozone and HCHO
...
for n in range(len(locations)): 
    ...
    axs[n].scatter(pod_data['INSTEP HCHO'], pod_data['INSTEP O3'], label='INSTEP', color=color[n])
    
    #add a line of best fit to each
    m, b = np.polyfit(pod_data['INSTEP HCHO'], pod_data['INSTEP O3'], 1)  # slope, intercept
    axs[n].plot(pod_data['INSTEP HCHO'], m*pod_data['INSTEP HCHO'] + b, color='black')
...
fig.text(0.5, 0.04, 'HCHO (ppb)', ha='center', va='center', fontsize=16)
...
savePath = os.path.join(Spath,'INSTEP_O3_HCHO')
