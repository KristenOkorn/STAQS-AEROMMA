# -*- coding: utf-8 -*-
"""
Created on Thu Oct  2 19:10:27 2025
...
locations = ['Whittier','AFRC','TMF','Caltech']
pods = ['YPODA7','YPODR9','YPODA2','YPODG5']
color = ["salmon", "skyblue", "lightgreen", '#f4a261'] 
...
for n in range(len(locations)): 
    ...
    x_max = pod_data['INSTEP tVOC'].max()
    y_max = pod_data['INSTEP O3'].max()
    
    #and use 0 for all mins - 80s are so points don't get cut off at the edges
    x_min = pod_data['INSTEP tVOC'].min()
    y_min = pod_data['INSTEP O3'].min()
    ...
    axs[n].scatter(pod_data['INSTEP tVOC'], pod_data['INSTEP O3'], label='INSTEP', color=color[n])
    
    #add a line of best fit to each
    #m, b = np.polyfit(pod_data['INSTEP tVOC'], pod_data['INSTEP O3'], 1)  # slope, intercept
    #axs[n].plot(pod_data['INSTEP tVOC'], m*pod_data['INSTEP tVOC'] + b, color='black')
...
savePath = os.path.join(Spath,'INSTEP_O3_tVOC_ratio')
